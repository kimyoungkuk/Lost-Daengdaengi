  ...
  <v-layout>
    <v-flex xs12 sm6 offset-sm3>
      <v-card>
        ...
        <v-card-title>
          <div>
            <span class="grey--text">연락처 : {{this.form.phone_num}}</span><br>
            <span>특징 : {{this.form.dog_feature}}</span><br>
...
export default {
  ...
  methods: {
    ...
    getBoardDetail () {
      this.$http.get(`http://202.30.31.91:8000/api/finderPosts/detail/${this.$route.params.id}`)
        .then(res => {
          console.log(res.data)
          this.form = res.data[0]
          console.log(this.form)
          this.form.find_time = this.$moment(this.form.find_time).format('LLLL')
